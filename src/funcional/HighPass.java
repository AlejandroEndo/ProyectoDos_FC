...
//		result = app.createImage(img.width, img.height, PConstants.RGB);
	...
	private void laplacian(PImage img, PImage result) {
		result.loadPixels();
		for (int i = 0; i < result.width; i++) {
			for (int j = 0; j < result.height; j++) {
				int index = i + j * img.width;

				int r = (int) (app.red(img.get(i, j)) - app.red(result.get(i, j)));
				int g = (int) (app.green(img.get(i, j)) - app.green(result.get(i, j)));
				int b = (int) (app.blue(img.get(i, j)) - app.blue(result.get(i, j)));
				
				result.pixels[index] = app.color(r,g,b);
			}
		}
	}
	...
	private void grayScale(PImage img) {
		...
		for (int i = 0; i < img.width; i++) {
			for (int j = 0; j < img.height; j++) {
				...
				int g = (int) app.green(img.get(i, j));
